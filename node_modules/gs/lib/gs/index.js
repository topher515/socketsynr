...
var World = this.World = function(name) {
...
var Ground = this.Ground = function() {
	this.passable = true;
}


var Critter = this.Critter = function(name) {
	...
	this.toJSON = function() {
		loc = self.world.critter_loc_by_id[self.id]
		if (loc) {
			return {'name':self.name,'x':loc[0],'y':loc[1]	}
		} else {
			return {'name':self.name,'x':null,'y':null}
		}
		
	}
}
util.inherits(Critter, events.EventEmitter)
...
var Player = this.Player = function(name) {
	Critter.call(this);
}
