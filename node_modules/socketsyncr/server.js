...
var SyncServer = exports.SyncServer = function() {
	...
	this.socketServer.on('connection', function(client) {
		...
		for (i in self.syncables) {
			client.send(
				shared.full_syncable_event_message(shared.NEW_SYNCABLE_,self.syncables[i])
			);
		};
	...
	*/
	this.register = function(syncable,client) {
		...
		/*syncable.on('change', function() {
			self.message_all_except(
				shared.lite_syncable_event_message(shared.SYNCABLE_CHANGED_,syncable), client.sessionId
			)
		})*/
		...
		if (client) {
			...
			self.message_all_except(
				shared.full_syncable_event_message(shared.NEW_SYNCABLE_,syncable), client.sessionId
			);
			self.message(
				shared.lite_syncable_event_message(shared.SERVER_SET_ID_,null,[temp_id,self.sync_id_next]), client.sessionId
			)
	...
	this.on_client_syncable_event(shared.NEW_SYNCABLE_,function(client,syncable_json,args) {
		
		temp_id = syncable_json[shared.SYNC_ID_];
